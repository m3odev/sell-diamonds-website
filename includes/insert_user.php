<?php
	...
	// get madh
	// $query="SELECT madh from donhang WHERE donhang.MaDh NOT IN(select madh from sanphamtrongdonhang ) order by asc";
	$query="SELECT DISTINCT m.MaDh from donhang as m
	join sanphamtrongdonhang as s
	on m.MaDh != s.Madh
	ORDER BY  m.MaDh DESC;";
	...
	check_errors($result_user,$query_user);
	
	if(isset($_SESSION['cart']))
	{
		
		foreach ($_SESSION['cart'] as $k => $v) //v là số lượng
		{
			if(isset($k))
			{
				...
				check_errors($result_db,$query_db);
			}
		}
	}
	unset($_SESSION['cart']);
